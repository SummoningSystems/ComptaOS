...
export function ExportView() {
  ...
  return <div className="p-6 max-w-5xl mx-auto flex flex-col gap-6">
    <div><h1 className="text-lg font-semibold text-vscode-text mb-1">Dossier expert-comptable</h1><p className="text-xs text-vscode-muted">Journal en partie double, balance générale, structure FEC contrôlée et justificatifs réunis dans une archive. Ce contrôle technique ne certifie pas la justesse fiscale des comptes.</p></div>
    ...
    <section className="bg-vscode-panel border border-vscode-border rounded-lg p-5 flex flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-3"><button disabled={loading !== null || blockers.length > 0} onClick={() => void downloadExpert("package")} className="px-4 py-2 bg-green-700 hover:bg-green-600 disabled:opacity-40 text-white text-sm rounded">{loading === "package" ? "Création…" : "Télécharger le dossier complet (.zip)"}</button><button disabled={loading !== null || blockers.length > 0} onClick={() => void downloadExpert("fec")} className="px-4 py-2 bg-blue-700 hover:bg-blue-600 disabled:opacity-40 text-white text-sm rounded">{loading === "fec" ? "Contrôle…" : "Télécharger le FEC (structure contrôlée)"}</button></div>
    </section>
