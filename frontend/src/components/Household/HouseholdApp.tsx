...
export function HouseholdApp({workspace,user,onLogout,onCreate}:{workspace:Company;user:AuthUser|null;onLogout:()=>Promise<void>;onCreate:()=>void}) {
  ...
  return <main className="hh"><header className="hh-top"><strong>ComptaOS</strong><select aria-label="Espace actif" value={workspace.id} onChange={async e=>{try{await setActiveCompanyApi(e.target.value);location.reload();}catch(error){setError(errorMessage(error));}}}>{workspaces.map(w=><option key={w.id} value={w.id}>{w.name}</option>)}</select>{admin&&<button onClick={onCreate}>+ Foyer</button>}<span className="hh-spacer"/><span>{user?.displayName??"Session locale"}</span><button onClick={()=>void onLogout()}>Déconnexion</button></header>
