...
export function Structure({scope}:{scope:string}) {
  ...
  return <div className="eco-work">
    ...
    <div className="eco-toolbar"><div className="eco-segment"><button aria-pressed={view==="map"} onClick={()=>setView("map")}>Carte</button><button aria-pressed={view==="list"} onClick={()=>setView("list")}>Liste</button></div>
      ...
      {view==="map"&&<label className="eco-inline">Zoom<select aria-label="Zoom de la carte" value={zoom} onChange={e=>setZoom(Number(e.target.value))}>{![.75,1,1.25].includes(zoom)&&<option value={zoom}>{Math.round(zoom*100)} %</option>}<option value={.75}>75 %</option><option value={1}>100 %</option><option value={1.25}>125 %</option></select></label>}
