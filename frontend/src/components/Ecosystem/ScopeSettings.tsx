...
export function ScopeSettings({spec,live=false}:{spec:ScopeTab;live?:boolean}) {
  ...
  const [target,setTarget]=useState(entity?.defaultTarget??"unassigned");
  const [assignments,setAssignments]=useState(entity?.treasuryAssignments??[]);
  ...
  return <div className="eco-work"><div className="eco-heading"><div><div className="eco-eyebrow">ÉCOSYSTÈME / {scopeName(spec.scope).toUpperCase()}</div><h1>Paramètres du périmètre</h1><p className="eco-muted">Ces réglages concernent uniquement {scopeName(spec.scope)}.</p></div></div>
    ...
    :entity&&<form className="eco-settings-form" onSubmit={async e=>{e.preventDefault();if(!name.trim())return;try{await save({...entity,name:name.trim(),archived,...(entity.kind==="company"?{accountingEnabled:accounting,vatEnabled:accounting&&vat}:entity.kind==="account"?{defaultTarget:target,opening,bankIdentifier,treasuryAssignments:assignments}:{})});setError("");setNotice("Réglages enregistrés. Les outils disponibles ont été mis à jour.");}catch(e){setError(errorText(e));}}}>
      ...
      <h2>Trésorerie dédiée</h2><p>Un compte mixte reste séparé de la trésorerie des entreprises. Une affectation dédiée est explicite et datée, indépendamment des titulaires.</p>
      {assignments.map((a,i)=><fieldset key={i}><label>Entreprise<select aria-label="Entreprise" required value={a.companyId} onChange={e=>setAssignments(assignments.map((v,j)=>j===i?{...v,companyId:e.target.value}:v))}><option value="">Choisir…</option>{entities.filter(e=>e.kind==="company"&&!e.archived).map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</select></label><label>Du<input type="date" required value={a.from} onChange={e=>setAssignments(assignments.map((v,j)=>j===i?{...v,from:e.target.value}:v))}/></label><label>Au (inclus, facultatif)<input type="date" value={a.to??""} onChange={e=>setAssignments(assignments.map((v,j)=>j===i?{...v,to:e.target.value||undefined}:v))}/></label><button type="button" onClick={()=>setAssignments(assignments.filter((_,j)=>i!==j))}>Retirer cette période</button></fieldset>)}
      <button type="button" onClick={()=>setAssignments([...assignments,{companyId:"",from:new Date().toISOString().slice(0,10)}])}>+ Période dédiée</button>
      <label>IBAN (facultatif)<input value={bankIdentifier} onChange={e=>setBankIdentifier(e.target.value)}/></label><label>Affectation par défaut<select value={target} onChange={e=>setTarget(e.target.value)}><option value="unassigned">Non affecté</option><option value="common">Vie commune</option>{entities.filter(e=>e.kind!=="account"&&!e.archived).map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</select></label>
