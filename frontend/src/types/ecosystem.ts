...
export interface Entity {
  ...
  bankIdentifier?:string; defaultTarget?:string; opening?:{date:string;cents:number};
}
...
export interface Movement {
  ...
  revision:number;reviewed:boolean;nature:"income"|"expense"|"refund";notes:string;deleted?:boolean;pending?:boolean;
...
export interface Document {id:string;name:string;kind:string;date:string;links:string[];movementIds:string[];fileName:string;mime:string;resource?:{workspaceId:string;kind:"attachment"|"invoice"|"quote";id:string}}
...
export interface Budget {id:string;target:string;category:string;cents:number}
export interface Recurring {id:string;label:string;accountId:string;cents:number;allocations:Allocation[];frequency:"monthly"|"quarterly"|"yearly";nextDate:string;active:boolean}
...
export interface Ecosystem {
  schemaVersion:1;id:string;name:string;revision:number;entities:Entity[];relations:Relation[];movements:Movement[];
  transfers:{id:string;fromId:string;toId?:string}[];documents:Document[];treatments:Treatment[];
  budgets:Budget[];recurring:Recurring[];feeds:Feed[];imports:{id:string;accountId:string;imported:number;skipped:number;at:string}[];
  history:{at:string;actor:string;action:string;details:unknown}[];
}
