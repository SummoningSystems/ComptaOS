...
function detectCategory(text: string): Category {
  ...
  if (/restaurant|brasserie|bistro|cafe|caf[eé]|repas|addition/.test(normalized)) return "restaurant";
  ...
  if (/materiel|mat[eé]riel|equipement|[eé]quipement/.test(normalized)) return "equipment";
  return "misc";
}
...
function detectSupplier(text: string): string {
  const ignored = /ticket|facture|note|duplicata|client|adresse|telephone|t[eé]l[.:]|siret|tva/i;
...
function detectInvoiceRef(text: string): string | undefined {
  return text.match(/(?:facture|ticket|note|re[cç]u|r[eé]f(?:[eé]rence)?)[\s#:n°-]*([a-z0-9][a-z0-9/_-]{2,})/i)?.[1];
}
...
export function parseReceiptTextLocally(rawText: string): ReceiptProposal {
  ...
  const amountTtc = findLastAmount(text, [
  ...
  const amountHt = findLastAmount(text, [new RegExp(`total\\s*ht[^\\d]{0,12}${AMOUNT}`, "gim"), new RegExp(`ht[^\\d]{0,12}${AMOUNT}`, "gim")]);
  const vatSplits = detectVatSplits(text);
  ...
  return {
    ...
    invoiceRef: detectInvoiceRef(text),
    amountHt,
    amountTtc,
    category: detectCategory(text),
    vatSplits: coherent ? vatSplits : [],
    confidence: coherent && fields >= 3 ? "high" : fields >= 2 ? "medium" : "low",
  };
}
