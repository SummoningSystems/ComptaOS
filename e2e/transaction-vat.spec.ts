...
test("enregistre puis ventile la TVA d'une transaction bancaire", async ({ page, request }) => {
  ...
  const created = await request.post("/api/workspaces/default/transactions", { data: transaction });
  ...
  let stored = await request.get("/api/workspaces/default/transactions");
  ...
  stored = await request.get("/api/workspaces/default/transactions");
