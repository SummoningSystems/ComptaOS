...
echo "=== [2/8] build frontend (Node 20) ==="
docker run --rm \
  --user "$(id -u):$(id -g)" \
  -e HOME=/tmp \
...
echo "=== [3/8] build backend (Node 20) ==="
docker run --rm \
  -v "$REPO/backend:/app" \
  -w /app \
  "$NODE_IMAGE" \
...
echo "=== [4/8] install nginx security headers ==="
